...
> = {
  ...
  'prized-yield': {
    ...
    description:
      'Principal stays safe while yield is directed to prize opportunities.',
...
export default function AppWorkspace({
  ...
  const { selectedGoalId, selectedVault, setSelectedGoalId, setSelectedVault } =
    useVaultContext();
  ...
  const [agentUiStatus, setAgentUiStatus] =
    useState<StatusState>(initialStatus);
  ...
  const [agentStatusData, setAgentStatusData] =
    useState<AgentStatusPayload | null>(null);
  const [agentDecisionData, setAgentDecisionData] =
  ...
  const resolveStrategyForMode = useCallback(
    ...
    [
      maxYieldStrategy?.address,
      safestStrategy?.address,
      selectedStrategyInfo?.address,
    ],
  );
  ...
  const getGoalMode = useCallback((goal: GoalInfo): GoalMode => {
  ...
  const loadGoals = useCallback(
    async (ids?: readonly bigint[]) => {
      ...
      try {
        ...
        const hasSelected = selectedGoalId
          ? fetched.some((goal) => goal.id === selectedGoalId)
          : false;
  ...
  const selectedGoalMode: GoalMode = selectedGoal
    ? getGoalMode(selectedGoal)
    : 'max-yield';

  const activeVault = useMemo(() => {
    if (selectedVault) return selectedVault;
    if (selectedGoal?.vault && isAddress(selectedGoal.vault))
      return getAddress(selectedGoal.vault);
    return undefined;
  }, [selectedGoal, selectedVault]);

  const depositVault = useMemo(() => {
    if (depositGoal?.vault && isAddress(depositGoal.vault))
      return getAddress(depositGoal.vault);
    return activeVault;
  ...
  const totalSaved = useMemo(
    () => goals.reduce((acc, goal) => acc + (goal.assets || 0n), 0n),
    [goals],
  );
  ...
  const hasDepositAmount =
    parsedDepositAmount !== null && parsedDepositAmount > 0n;
  const canDeposit =
  ...
  const maxWithdraw = useMemo(() => {
    if (!totalAssets || !totalShares || !userShares || totalShares === 0n)
      return 0n;
    return (userShares * totalAssets) / totalShares;
  }, [totalAssets, totalShares, userShares]);

  const maxWithdrawFormatted = formatAmount(
    maxWithdraw,
    appConfig.usdcDecimals,
  );
  const hasWithdrawAmount =
    parsedWithdrawAmount !== null && parsedWithdrawAmount > 0n;
  const canWithdraw =
  ...
  const estimatedYieldFormatted = formatAmount(
    estimatedYield,
    appConfig.usdcDecimals,
  );
  ...
  const detailSaved = selectedGoal
    ...
    : '0.00';
  const detailProgress = formatProgress(
    selectedGoal?.assets,
  ...
  const createdDate = selectedGoal?.createdAt
    ...
    : '-';
  const selectedStrategyLabel = selectedGoal
    ? getStrategyLabel(selectedGoal.strategy)
    : '-';
  ...
  const handleConnect = async (connectorIndex: number) => {
    ...
    try {
      ...
      setStatus({
        message: error.shortMessage || error.message || 'Connect failed.',
        kind: 'error',
      });
    }
  };
  ...
  const ensureNetwork = useCallback(async () => {
    ...
    try {
      ...
      const ethereum = (
        window as { ethereum?: { request?: (args: unknown) => Promise<void> } }
      ).ethereum;
  ...
  useEffect(() => {
    ...
    ensureNetwork().catch((err) => {
      console.error(err);
      setStatus({
        message: 'Please switch to the configured network.',
        kind: 'error',
      });
    });
  ...
  const handleCreateGoal = async () => {
    ...
    if (!isAddress(resolvedStrategy)) {
      setStatus({
        message: 'Selected strategy address is invalid.',
        kind: 'error',
      });
      return;
    }
    ...
    try {
      ...
      const refreshedIds =
        (refreshed.data as readonly bigint[] | undefined) || [];
      ...
      const error = err as { shortMessage?: string; message?: string };
      setStatus({
        message: error.shortMessage || error.message || 'Goal creation failed.',
        kind: 'error',
      });
    }
  };
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const error = err as { shortMessage?: string; message?: string };
      setStatus({
        message: error.shortMessage || error.message || 'Approval failed.',
        kind: 'error',
      });
    }
  };
  ...
  const handleDeposit = async () => {
    if (!depositVault || !address) {
      setStatus({
        message: 'Connect wallet and select a vault.',
        kind: 'error',
      });
      return;
    }
    ...
    try {
      ...
      const error = err as { shortMessage?: string; message?: string };
      setStatus({
        message: error.shortMessage || error.message || 'Deposit failed.',
        kind: 'error',
      });
    }
  };
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      const error = err as { shortMessage?: string; message?: string };
      setStatus({
        message: error.shortMessage || error.message || 'Withdrawal failed.',
        kind: 'error',
      });
    }
  };
    ...
    try {
      ...
      if (statusRes.ok) {
        ...
        setAgentDecisionData(
          (prev) => prev || statusPayload.lastDecision || null,
        );
      }
      ...
      console.error(err);
      setAgentUiStatus({
        message: 'Could not load automation data.',
        kind: 'error',
      });
    } finally {
  ...
  const handleRunAgent = async () => {
    ...
    try {
      setAgentUiStatus({
        message: 'Requesting manual agent cycle...',
        kind: 'loading',
      });
      ...
      if (payload.execution) {
        setAgentUiStatus({
          message: 'Automation run executed.',
          kind: 'success',
        });
      } else if (payload.skipped) {
        setAgentUiStatus({
          message: `Run skipped: ${payload.reason || 'no action'}.`,
          kind: 'success',
        });
      } else if (payload.decision) {
        setAgentUiStatus({
          message: 'Decision generated. No execution performed.',
          kind: 'success',
        });
      } else {
        setAgentUiStatus({
          message: 'Automation request completed.',
          kind: 'success',
        });
      }
  ...
  const walletStatus = mounted
    ? isConnected && address
      ...
      : 'Not connected'
    : '...';
  const chainLabel = mounted
    ? chain?.name || appConfig.chainName
    : appConfig.chainName;
  const networkMismatch =
    mounted && isConnected && chain?.id !== appConfig.chainId;
  ...
  const selectedGoalTickets = selectedGoal
    ? selectedGoal.assets / prizeTicketUnit
    : 0n;
  ...
  const factoryExplorerUrl = factoryAddress
    ? `${explorerBaseUrl}/address/${factoryAddress}`
    : '';
  const prizePoolExplorerUrl = appConfig.prizePoolAddress
    ? `${explorerBaseUrl}/address/${appConfig.prizePoolAddress}`
    : '';
  const usdcExplorerUrl = usdcAddress
    ? `${explorerBaseUrl}/address/${usdcAddress}`
    : '';
      ...
      <aside className='sidebar'>
        ...
        <div className='vault-summary'>
          ...
          <p style={{ margin: '12px 0 0', color: 'var(--muted)' }}>
            {goalModeCounts['prized-yield']} goals are using principal-protected
            prize mode.
          </p>
        </div>
      ...
      <main>
        ...
        <section className='kpi-strip'>
          ...
          <div className='kpi-card'>
            ...
            <span
              className={`kpi-chip ${agentStatusLabel === 'Disabled' ? 'warn' : 'ok'}`}
            >
              {agentStatusLabel}
            </span>
          </div>
        ...
        {activeView === 'dashboard' ? (
          <section className='grid dashboard-grid'>
            <div className='card dashboard-goal-progress-card'>
              ...
              {goals.length === 0 ? (
                <div className='empty-state'>
                  No goals yet. Create one to start your savings journey.
                </div>
              ) : (
                    ...
                    return (
                      <div
                        ...
                        onClick={() => {
                          ...
                          if (isAddress(goal.vault))
                            setSelectedVault(getAddress(goal.vault));
                        }}
                        ...
                        <div className='goal-meta'>
                          <div>
                            ...
                            <div className='goal-subtitle'>
                              {GOAL_MODE_CONFIG[mode].shortLabel}
                            </div>
                          </div>
                  ...
                  {activities.map((item) => (
                    <div className='activity-item' key={item.id}>
                      <div style={{ fontWeight: 600 }}>
                        {item.type} $
                        {formatAmount(item.amount, appConfig.usdcDecimals)}
                      </div>
                      <div style={{ color: 'var(--muted)', marginTop: 4 }}>
                        {item.goalName || shortAddress(item.vault)} -{' '}
            ...
            <div className='card dashboard-automation-pulse-card'>
              ...
              {!appConfig.agentUrl ? (
                <div className='empty-state'>
                  Set NEXT_PUBLIC_AGENT_URL to enable live automation telemetry.
                </div>
              ) : isAgentLoading ? (
                ...
                <div className='detail-grid'>
                  <div className='detail-row'>
                    <span>Executor</span>
                    <span>
                      {agentStatusData?.executor ||
                        appConfig.agentExecutor ||
                        '-'}
                    </span>
                  </div>
                  <div className='detail-row'>
                  ...
                  <div className='detail-row'>
                    ...
                    <span>
                      {agentDecisionData?.selectedStrategy?.name || '-'}
                    </span>
                  </div>
                </div>
              )}
              {agentUiStatus.message ? (
                <div className='status'>{agentUiStatus.message}</div>
              ) : null}
            </div>
          </section>
        ) : null}
        ...
        {activeView === 'goals' ? (
          <section className='grid-split goals-grid'>
            <div className='card goals-list-card'>
              ...
              {goals.length === 0 ? (
                ...
                <div className='goals-list'>
                  {goals.map((goal) => {
                    const saved = formatAmount(
                      goal.assets,
                      appConfig.usdcDecimals,
                    );
                    const targetFormatted = formatAmount(
                      goal.targetAmount,
                      appConfig.usdcDecimals,
                    );
                    const progress = formatProgress(
            ...
            <div className='card goal-withdraw-card'>
              ...
              {!selectedGoal ? (
                <div className='empty-state'>
                  Select a goal to open mode-specific detail.
                </div>
              ) : (
                <>
                  <div className='detail-grid'>
                    ...
                    <div className='detail-row'>
                      <span>Vault</span>
                      <span>
                        {activeVault ? shortAddress(activeVault) : '-'}
                      </span>
                    </div>
                    <div className='detail-row'>
                      <span>Strategy</span>
                      <span className='detail-strategy'>
                  ...
                  <div className='goal' style={{ paddingTop: 8 }}>
                    <div
                      className='progress-track'
                      style={
                        { '--progress': detailProgress } as React.CSSProperties
                      }
                    >
                      <span />
                    </div>
                  ...
                  <p style={{ color: 'var(--muted)', margin: '12px 0' }}>
                    Saved: ${detailSaved} of ${detailTarget} - PPS:{' '}
                    {ppsFormatted}
                  </p>
                  ...
                  {selectedGoalMode === 'max-yield' ? (
                    <div className='mode-block'>
                      <div className='mode-summary'>
                        ...
                        <strong>
                          Autonomous optimization with dynamic strategy
                          rotation.
                        </strong>
                      </div>
                      <div className='detail-grid'>
                        ...
                        <div className='detail-row'>
                          ...
                          <span>
                            {maxYieldStrategy ? maxYieldStrategy.name : '-'}
                          </span>
                        </div>
                        <div className='detail-row'>
                          <span>Yield (est.)</span>
                  ...
                  {selectedGoalMode === 'prized-yield' ? (
                    <div className='mode-block'>
                      <div className='mode-summary'>
                        ...
                        <strong>
                          Principal-protected savings, yield flows to prize
                          mechanics.
                        </strong>
                      </div>
                      <div className='detail-grid'>
                        ...
                        <div className='detail-row'>
                          ...
                          <span>
                            {shortAddress(appConfig.prizePoolAddress)}
                          </span>
                        </div>
                        <div className='detail-row'>
                  ...
                  {selectedGoalMode === 'strategy-select' ? (
                    <div className='mode-block'>
                      <div className='mode-summary'>
                        ...
                        <strong>
                          Manual strategy mode. User selects and keeps a fixed
                          route.
                        </strong>
                      </div>
                      <div className='detail-grid'>
                        ...
                        <div className='detail-row'>
                          ...
                          <span>
                            {getStrategyRiskLabel(selectedGoalAprBps)}
                          </span>
                        </div>
                        <div className='detail-row'>
          ...
          <section className='grid settings-grid'>
            ...
            <div className='card'>
              ...
              <p className='goal-subtitle' style={{ marginTop: 12 }}>
                Use conservative defaults for prize mode and explicit strategy
                lock for higher-volatility routes.
              </p>
            </div>
              ...
              <div className='detail-grid'>
                ...
                <div className='detail-row'>
                  ...
                  <span className='detail-mono'>
                    {appConfig.agentUrl || 'Not configured'}
                  </span>
                </div>
                <div className='detail-row'>
                  <span>Executor</span>
                ...
                <div className='detail-row'>
                  ...
                  <span>
                    {appConfig.agentExecuteApiKey ? 'Configured' : 'Not set'}
                  </span>
                </div>
              </div>
              <div className='mode-actions'>
        ...
        <div className='modal-backdrop' onClick={() => setIsConnectOpen(false)}>
          <div className='modal' onClick={(event) => event.stopPropagation()}>
            ...
            <p style={{ margin: 0, color: 'var(--muted)' }}>
              Choose a wallet provider. We will switch to {appConfig.chainName}{' '}
              automatically.
            </p>
                    ...
                    {goalModeOptions.map((option) => (
                      ...
                      >
                        <div className='mode-option-headline'>
                          ...
                          {option.isDefault ? (
                            <span className='goal-mode-default'>Default</span>
                          ) : null}
                        </div>
                        ...
                        <div className='detail-row'>
                          ...
                          <span>
                            {selectedStrategyInfo
                              ? `${(selectedStrategyInfo.aprBps / 100).toFixed(2)}%`
                              : '-'}
                          </span>
                        </div>
                        <div className='detail-row'>
                          <span>Risk level</span>
                          <span>
                            {getStrategyRiskLabel(selectedStrategyInfo?.aprBps)}
                          </span>
                        </div>
                        <div className='detail-row'>
                          <span>Liquidity</span>
              ...
              {goalCreateStep === 3 ? (
                <>
                  <div className='mode-summary'>
                    ...
                    <strong>
                      {goalName || 'New Goal'} ·{' '}
                      {GOAL_MODE_CONFIG[goalModeDraft].label} ·{' '}
          ...
          <div className='modal' onClick={(event) => event.stopPropagation()}>
            ...
            <form className='form' onSubmit={(event) => event.preventDefault()}>
              <div style={{ color: 'var(--muted)' }}>
                Mode:{' '}
                {depositGoal
                  ? GOAL_MODE_CONFIG[getGoalMode(depositGoal)].label
                  : '-'}
              </div>
              <div style={{ color: 'var(--muted)' }}>
                Strategy:{' '}
                {depositGoal ? getStrategyLabel(depositGoal.strategy) : '-'}
              </div>
              <div>
              ...
              {networkMismatch ? (
                <div className='status'>
                  Switch to {appConfig.chainName} to continue.
                </div>
              ) : null}
              {status.message ? (
                <div className='status'>{status.message}</div>
